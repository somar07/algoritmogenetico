...
def get_graph_which_stisfy_bandwidth(graph, src, dst, min_band_width):
    adj = nx.DiGraph(graph, weight='Bandwidth')
    remove_lower_edges = np.vectorize(
        lambda x: x if x >= min_band_width else 0)
    return nx.DiGraph(remove_lower_edges(adj))
...
def embed_request_in_cluster(request_graph, physical_graph, center_node, max_visit, beta=3, verbose=False, check_verbose=False, embedding_verbose=False):
    ...
    It uses BFS to traverse graph and in each iteration it selects the best nodes at a depth.

    Returns: embedding was successful or not
    """
    max_depth = beta
    
    q = nx.Graph(request_graph)
    ...
    for node in sorted_request_nodes:
        ...
        while queue:
            ...
            if can_embed(current_node, i, request_graph, physical_graph, verbose=check_verbose):
                try:
                    embed_to_physical(current_node, i, request_graph,
                                      physical_graph, verbose=embedding_verbose)
                    node_embedded = True
                    break
                except nx.NetworkXNoPath:  # In a rare case of having no path after embedding some bandwidths
                    free_embedded_request_node(physical_graph, request_graph, i)

            if depth == max_depth:
                continue

            node_edges = physical_graph.edges(center_node, data=True)
            node_edges = node_edges if len(
                node_edges) <= max_visit else node_edges[:max_visit_in_every_depth]
        ...
        if not node_embedded:
            return False
        elif node_embedded and i == request_graph.number_of_nodes() - 1:
            return True
...
def embed_request(cluster_center, physical, request_graph, alpha=30, verbose=True):
    ...
    max_visit_nodes = len(request_graph) * alpha
...
def graphSAGE_embed(physical_graph, cluster_index, request_graph, max_try=0.5, N_CLUSTERS=4, verbose=False):
    embeddeds = []
    embedded = False
    for i in range(N_CLUSTERS):
        ...
        while not request_embedded and j != 0:
            ...
            request_embedded, physical_graph, request_graph = embed_request(
                selected_node, physical_graph, nx.Graph(request_graph).to_undirected(), verbose=verbose)
            j -= 1
